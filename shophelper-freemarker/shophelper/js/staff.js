window.TApi.staff = (function(_TApi)
{
    ...
    _VoidInputDone.onclick = function()
    {
        ...
        {
            _TApi.prompt('${languageMap.PleaseInputVoidNum}', null);
            return;
        }
        else{
            Popup.close(true);
	...
	{
		_TApi.confirm(['${languageMap.LogoutConfirm}'], function(yes)
		{
			if(yes)
			{
				TApi.staff.logout();
    ...
    {
        _TApi.confirm(['${languageMap.ZReadConfirm}'], function(yes)
        {
            if(yes)
            {
