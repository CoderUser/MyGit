window.TApi.settle = (function(_TApi)
{
	...
	function handleTenderClick(el)
	{
		...
		{
			_TApi.prompt( tenderInfo.tenderName + ' ${languageMap.NotAllowRefund}', null); return;
		}
		...
		/*else if(tenderCode == TENDERCODE_TEMPSALES)
		{
			...
			if(getTenderCount() > 0)
			{
				_TApi.confirm('${languageMap.ClearOtherTenderPrompt}', function(ok)
				{
					if(ok) {
	...
	{
		UserMsgTitle.innerText = '${languageMap.Coupon}';
		_showBottom(divGcc);
		setTimeout(function () {
			_divGccScroll.refresh();
	    }, 100);
	...
	function addCouponRow()
	{
		...
		input.placeholder = '${languageMap.PleaseInputCouponNo}';
	...
	orderConfirmButton.onclick = function()
	{
		...
		{
			Toast.show("${languageMap.PaidAmountNotEnough}");
			return;
		}
		...
		{
			_TApi.confirm('${languageMap.CompleteOrderConfirm}', function(ok)
			{
				if(ok) submitOrder();
			});
		}
	...
	function submitOrder()
	{
		...
		{
			if(getPaidAmount() <= 0) {
				Toast.show("${languageMap.PayAmountCannotBeZero}");
				return;
			}
			UserMsgTitle.innerText = '${languageMap.FlowMethod}';
	...
	function showGcCpInputDialog(tenderCode, tenderType)
	{
		...
		switch (tenderType)
		{
			case TENDERTYPE_GIFTCERT:
				popupTitle.innerHTML = "${languageMap.SettleGiftCert}"; //TODO
					...
					if (resultCode == RESULT_SUCCESS) {
						...
						switch (resultCode) {
							...
							case RESULT_CPGC_DUPLICATE:
								_TApi.Toast.show("${languageMap.CardNoDuplicate}");
								break;
							case RESULT_CPGC_EXCEEDBATCHLMT:
								_TApi.Toast.show("${languageMap.ExceedBatchLimit}");
								break;
						}
					}
				});
	...
	function initPromotionInput()
	{
		...
		function getBonusRedeemDescHtml(index, promInput)
		{
			...
			promDescDiv.innerHTML = '<span>[' + promInput.promid + '] ' + promInput.promdesc + '</span><span>${languageMap.RedeemBonus}: '
		...
		function getBonusRedeemInputHtml(index, pkgRedeem, pkgLess, successPkg)
		{
			...
			div.innerHTML = '<span class="manual-bonusdesc">${languageMap.Redeem}: '
							+ pkgRedeem
							+ '(${languageMap.pts})'
							+ '<em></em>${languageMap.MaxPackages}: '
							+ successPkg
		...
		function getQuotaLimitInputHtml(index, totalAvailQty, promLessArray)
		{
			...
			div.innerHTML = '<span style="font-weight:bold;">${languageMap.TotalAvailQty}: ' + totalAvailQty + '</span>';
			for (key in quotaLimitItems)
			{
				div.innerHTML += '<span class="manual-quota">'
								 ...
								 + '; ${languageMap.AvailQty}: '
								 + quotaLimitItems[key].availQty
								 + '; ${languageMap.UseQty}: <input type="number" class="manual-quotainput" value="0" data-index="'
								 + index
								 + '" data-itemcode="'
								 + quotaLimitItems[key].itemCode
								 + '" data-itemorg="'
		...
		function getInputRow(allowDuplicate, inputData)
		{
			...
			div.className = "g-dis-flex";
			if (inputData)
				div.innerHTML = '<input type="text" class="a-inputvalue" placeholder="'+"${languageMap.PleaseInputNo}"+'" value="'
								+ inputData.num
				...
				div.innerHTML = '<input type="text" class="a-inputvalue" placeholder="'+"${languageMap.PleaseInputNo}"+'"/>*<input type="number" class="a-inputqty'
								+ (allowDuplicate ? ' a-numpad' : '')
		...
		function displayPromManual()
		{
			...
			if (issueFragment.hasChildNodes())
			{
				...
				issueWrapper.innerHTML = "<span class='manual-input-header'>${languageMap.Issue}</span>"
			...
			if (bonusRedeemFragment.hasChildNodes())
			{
				...
				bonusRedeemWrapper.innerHTML = "<span class='manual-input-header'>${languageMap.BousRedeem}</span>"
			...
			if (quotaLimitFragment.hasChildNodes())
			{
				...
				quotaLimitWrapper.innerHTML = "<span class='manual-input-header'>${languageMap.QuotaLimit}</span>"
		...
		promInputContainer.addEventListener("change", function(e)
		{
			...
			function callback(resultCode, responseContent)
			{
				if (resultCode == _TApi.promotion.RESULT_SUCCESS)
				{
					...
					save.innerHTML = "${languageMap.Save}: $" + newSave;
				...
				else
				{
					...
					switch (resultCode)
					{
						case _TApi.promotion.RESULT_SERVERERROR:
							_TApi.Toast.show("${languageMap.ServerError}");
							break;
						case _TApi.promotion.RESULT_BONUSREDEEM_EXCEEDMAXPKG:
							_TApi.Toast.show("${languageMap.ExceedMaxPackages}");
							break;
						case _TApi.promotion.RESULT_QUOTALIMIT_EXCEEDLIMIT:
							_TApi.Toast.show("${languageMap.ExceedQtyLimit}");
							break;
					}
				}
			}
