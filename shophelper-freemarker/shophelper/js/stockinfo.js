	...
	function retrieveListLeft()
	{
		TAjax.request({
			...
			success: function(response)
			{
				...
				{
					TApi.prompt("${languageMap.NoRecord}", function(){
						AppContext.goBack();
					});
	...
	_stockInfo.renderListItem = function(stockInfo)
	{
		...
		for (var key in data) 
		{
		    ...
		    for (i=0; i<store.color.length; i++)
			{
				...
				detailType.innerText = '${languageMap.QOH}';
				...
				detailType2.innerText = '${languageMap.INT}';
				...
				detailType3.innerText = '${languageMap.Reserve}';
