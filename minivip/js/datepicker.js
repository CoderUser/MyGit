// @define TApi.DatePicker
/*
 * LCalendar日期控件
 */
window.TApi.DatePicker = (function() {
		...
		bindEvent: function(type) {
			...
			function setDateGearTooth() {
				...
				if (date_yy && date_yy.getAttribute("val")) {
					...
					//i 当前节点前后需要展示的节点个数
					for (var i = 0; i <= passY - 1; i++) {
						itemStr += "<div class='tooth'>" + (_self.minY + i) + "</div>";
					}
				...
				if (date_mm && date_mm.getAttribute("val")) {
					...
					//j 当前节点前后需要展示的节点个数
					for (var j = 0; j < maxM - minM + 1; j++) {
						itemStr += "<div class='tooth'>" + (minM + j + 1) + "</div>";
					}
			...
			function setTimeGearTooth() {
				...
				if (time_hh && time_hh.getAttribute("val")) {
					...
					for (index = 0; index <= 23; index++) {
						html += "<div class='tooth'>" + index + "</div>";
					}
					time_hh.innerHTML = html;
				...
				if (time_mm && time_mm.getAttribute("val")) {
					...
					time_mm.innerHTML = html;
					time_mm.style["-webkit-transform"] = 'translate3d(0,' + (8 - mmVal * 2) + 'em,0)';
					time_mm.setAttribute('top', 8 - mmVal * 2 + 'em');
				}
			}
			function closeMobileCalendarOnTouch()
			{
				// close calendar when touch modal
				_self.gearDate.onclick = function(e)
				{
					if (e.target == _self.gearDate) closeMobileCalendar(e);
				};
			}
			//求月份最大天数
			...
			function gearTouchStart(e) {
				...
				// init parameter in case not trigger gearTouchMove, fixed in 2016-07-19
				target["new_" + target.id] = target["old_" + target.id];
				target["n_t_" + target.id] = target["old_" + target.id];
				target["pos_" + target.id] = target["o_d_" + target.id];
			}
			//手指移动
			...
			function rollGear(target) {
				...
				target["int_" + target.id] = setInterval(function() {
					...
					var minTop, gearVal, date_yy, yyVal;
					switch (target.dataset.datetype) {
						case "date_yy":
							...
							if (stopGear) {
								gearVal = Math.abs(pos - 8) / 2;
								setGear(target, gearVal);
								clearInterval(target["int_" + target.id]);
							}
							break;
						case "date_mm":
							date_yy = _self.gearDate.querySelector(".date_yy");
							//得到年份的值
