window.TApi.stock = (function(_TApi)
{
	...
	//VNavigatorBar.updateNavTitle("[$Stock$]");
	...
	{
		UserMsgTitle.innerText = '[$SelectZone$]';
	...
	{
		UserMsgTitle.innerText = '[$SelectStore$]';
	...
	btnGo.onclick = function()
	{
		...
		if(!style)
		{
			TApi.prompt("[$PleaseInputItemCode$]", null);
			return;
		}
		if(!lastStoreCodes)
		{
			TApi.prompt("[$PleaseSelectStoreCode$]", null);
			return;
		}
	...
	function confirmSelectStore()
	{
		...
		if(!storeBtn.innerHTML) storeBtn.innerHTML = '[$SelectStore$]';
	...
	function confirmSelectStorePref()
	{
		...
		if(!storeZoneBtn.innerHTML) storeZoneBtn.innerHTML = '[$SelectStoreZone$]<i></i>';
	...
	function loadStoreListPref()
	{
		...
		store.load(function(records, operation, success) 
		{
		    ...
		    for(var i=0; i<records.length;i++)
	    	{
				...
				input.placeholder = '[$PleaseInputStoreZone$]';
