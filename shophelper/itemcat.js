window.TApi.itemcat = (function(_TApi, _itemcat)
{
	...
	function initClickEvent() {
		...
		_staffLoginDone.onclick = function()
		{
			...
			{
				Toast.show('[$StaffCodeCannotEmpty$]');
				_staffCodeIpt.focus();
			}
			else if(ls_pwd.length == 0)
			{
				Toast.show('[$PwdCannotEmpty$]');
				_staffPwdIpt.focus();
			}
			else
			{
	...
	function staffLogin(staffcode, password, callback)
	{
		TAjax.request({
			...
			callback: function(options, success, response)
			{
				...
				{
					TApi.prompt("[$StaffCode$] " + responseObj.errorMessage + " [$Notfound$]", null);
					if(callback) callback(false);
					return;
				}
				else if (responseObj.errorCode == ERROR_PASSWORDNOTCORRECT)
				{
					TApi.prompt("[$PasswordNotCorrect$]", null);
					if(callback) callback(false);
					return;
				}
				else
