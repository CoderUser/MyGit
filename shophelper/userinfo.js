window.TApi.userinfo = (function(_TApi)
{
	...
	for (i = 0; i < UserMsgLi01.length; i++)
	{
		UserMsgLi01[i].onclick = function()
		{
			...
			if (ThisText.indexOf("[$UserName$]") >= 0)
			{
				_showBottom(MdyUser);
			...
			else if (ThisText.indexOf("[$Sex$]") >= 0)
			{
				_showBottom(MdySex);
			...
			else if (ThisText.indexOf("[$Birthday$]") >= 0)
			{
	...
	for (i = 0; i < UserMsgLi02.length; i++)
	{
		...
		{
			var ThisText = this.innerText;
			if (ThisText.indexOf("[$Mobile$]") >= 0)
			{
				_showBottom(MdyTelEmail);
				MdyTelEmailVal.type = 'tel';
				MdyTelEmailVal.placeholder = '[$NewMobile$]';
			...
			else if (ThisText.indexOf("[$Email$]") >= 0)
			{
				_showBottom(MdyTelEmail);
				MdyTelEmailVal.type = 'email';
				MdyTelEmailVal.placeholder = '[$NewEmail$]';
	...
	UserMsgConfirm.onclick = function()
	{
		if (MdyUser.style.display == 'block')
		{
			...
			{
				Toast.show('[$Name1Null$]');
				MdyUserInput1.focus();
				return false;
			}
			else if (MdyUserInput2.value == '')
			{
				Toast.show('[$Name2Null$]');
				MdyUserInput2.focus();
				return false;
			}
			else
	...
	function checktel()
	{
		...
		{
			TApi.prompt("[$PleaseTelCode$]", null);
			return false;
		}
	...
	_userInfo.regCheck = function()
	{
		...
		{
			TApi.prompt("[$UserNameCannotBeEmpty$]", null); return;
		}
		else if(!_getName1())
		{
			TApi.prompt("[$GivenNameCannotBeEmpty$]", null); return;
		}
		else if(!_getName2())
		{
			TApi.prompt("[$SurnameCannotBeEmpty$]", null); return;
		}
		else if(!_getSex())
		{
			TApi.prompt("[$SexCannotBeEmpty$]", null); return;
		}
		else if(!birthYYYY && !birthMM && !birthDD)
		{
			TApi.prompt("[$BirthdayCannotBeEmpty$]", null); return;
		}
		else if(!_getTelAreaCode())
		{
			TApi.prompt("[$PleaseTelCode$]", null); return;
		}
		else if(UserTel.innerHTML.trim().length == 0 && UserEmail.innerHTML.trim().length == 0)
		{
			TApi.prompt("[$TelOrEmailNotValid$]", null); return;
		}

		TApi.confirm("[$RegConfirm$]", function(yes)
		{
			if(yes)
			{
		...
		TAjax.request({
	          ...
	          callback: function(options, success, response)
	          {
	              ...
	              if(errorCode > 0)
	              {
		              ...
		              {
			              case 1:
				              TApi.prompt('[$DuplicateMobile$]', null);
				              return;
			              case 2:
				              TApi.prompt('[$DuplicateEmail$]', null);
				              return;
			              case 3:
				              TApi.prompt('[$InvalidRegId$]', null);
				              return;
			              case 4:
				              TApi.prompt('[$CreateFailed$]', null);
				              return;
			              default:
	              ...
	              {
		              TApi.confirm("[$RegSuccessLoginNow$]?", function(yes)
		              {
			              if(yes)
			              {
