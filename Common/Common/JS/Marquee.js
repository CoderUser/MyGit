//滚动图片构造函数
function Marquee(scrollContId, arrLeftId, arrRightId, dotListId, listType)
{
	this.rootEl = null;	//root element
	...
	this.listType = listType; //列表类型  number数字，img缩略图，string字符串 默认为空
	...
	this.speed = 5; //移动速度(单位毫秒，越小越快)
	this.space = 20; //每次移动像素(单位px，越大越快)
	this.scrollWidth = 5; //横向滚动宽度
	this.minMove = 25;		//最小移动距离(%)
	this.upright = false; //垂直的滚动
	this.pageIndex = 0;
	this.pageNum = 4;
	this.autoPlay = false;
	this.autoPlayTime = 1; //秒
...
Marquee.prototype = {
	...
	initialize: function()
	{
		...
		if(this.desci)
		{
			var _desc =  document.createElement('div');
				_desc.id = 'img-desc';
				_desc.className = 'img-desc';
				_desc.innerHTML = this.stripDiv.getElementsByTagName('img')[0].getAttribute('alt') + '<span><em>1</em> / ' + this.pageNum+ '</span>';
				this.stripDiv.appendChild(_desc)
		}
		//dot
		if (this.dotListId)
		{
			...
			if (this.dotListObj)
			{
				for (var i = 0, tempObj; i < pages; i++)
				{
					...
					else if (this.listType == 'string')
					{
						tempObj.innerHTML = String(this.stripDiv.getElementsByTagName('img')[i].getAttribute('title'));
					}
					else if (this.listType == 'img')
					{
						tempObj.innerHTML = String(this.stripDiv.getElementsByTagName('img')[i].parentNode.innerHTML);
					}
					else
					{
						tempObj.innerHTML = '';
	...
	move: function(num, quick)
	{
		...
		if (Math.abs(num) == 0)
		{
			...
			this.accountPageIndex();
			if (typeof(this.onpagechanged) === 'function') //added
			{
	...
	accountPageIndex: function()
	{
		...
		for (var i = 0; i < this.dotObjArr.length; i++)
		{
			if (i == this.pageIndex)
			{
		...
		if(this.desci && document.getElementById('img-desc'))
		{
			document.getElementById('img-desc').innerHTML = this.stripDiv.getElementsByTagName('img')[pageIndex].getAttribute('alt') + '<span><em>' + (pageIndex+1) + '</em> / ' + this.pageNum+ '</span>';
		}
	},
	iPadX: 0,
	iPadLastX: 0,
	...
	_touchend: function()
	{
		...
		//this._state = 'ready';
		var cX = this.iPadX - this.iPadLastX;
		var _minMove = this.pageWidth / 100 * this.minMove;
		if (cX < _minMove * -1)
		{
			this.rightEnd();
		}
	...
	{
		var _el;
		if (this.rootEl) _el = this.rootEl.querySelector("#" + objName);
		if (_el) return _el;
