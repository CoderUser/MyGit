window.TApi.itemdetail = (function(_TApi)
{
	...
	Listener.addDocumentListener('touchend', function (e)
	{
		...
		{
			Tips.show(this,'<a href="#VUser">${languageMap.MyUser}</a><a href="#VHome">${languageMap.VHome}</a><a href="#VFavourite">${languageMap.VFavourite}</a><a href="#VBrowseHistory">${languageMap.Footprint}</a><div class="triangle-down dbl"></div>');
		};
		...
		if(_PosY >= _maxPosY - 1)
		{
			...
			_TApi.CMS.embed(DetailCon, {
				...
				callbackfail: function()
				{
					...
					DetailCon.innerHTML = '<p class="pull">${languageMap.NoDetail}</p>';
				}
			});
	...
	var _initMarquee = function()
		...
		_addToCart = function(_itemOrgId, _plu, _qty, _photoEl)
		{
            ...
            var cartLink = _TApi.$id("cart-icon");
