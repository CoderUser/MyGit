window.TApi.suggest = (function(_TApi)
{
	...
	VNavigatorBar.updateTitle("[$Suggest$]");
	...
	function checkInput()
	{
		...
		if (!_content.value)
		{
			_TApi.Toast.show("[$ContentInvalid$]");
			return false;
		}
		return true;
	}
	...
	function deleteSuggest()
	{
		TAjax.request({
			...
			success: function(resp)
			{
				...
				{
					case 0:
						_TApi.Toast.show("[$DeleteSuccess$]");
						AppContext.goBack();
						break;
					default:
			...
			success: function(resp)
			{
				...
				switch (respObj.errorCode)
				{
					case 0:
						...
						break;
					case -1:
						_TApi.Toast.show("[$SuggestNoDuplicate$]");
						break;
					default:
						_TApi.Toast.show(respObj.errorMessage);
				}
	...
	_buttonConfirm.onclick = function()
	{
		...
		uploadImage(function()
		{
			...
			{
				_TApi.Toast.show("[$UpdateSuccess$]");
				AppContext.goBack();
			});
		});
	};
	FileImg.onchange = function()
	{
		...
		if (count > -1)
		{
			...
			for (var index = 0; index < count; ++index)
			{
				...
				{
					_TApi.Toast.show("[$UploadTooMuch$]");
					break;
				}
				// compress image
